...
impl TokenRotator {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        info!("Fetching trending tokens for rotation...");
        let tokens = match self.jupiter_client.fetch_trending_tokens().await {
            Ok(tokens) => tokens,
            Err(e) => {
                error!("Failed to fetch trending tokens: {}", e);
                return Err(anyhow!("Failed to fetch trending tokens: {}", e));
            }
        };
        
        // Filter out tokens with low volume and WSOL (since we use it as tokenA)
        ...
        if filtered_tokens.is_empty() {
            error!("No suitable tokens found for rotation after filtering");
            return Err(anyhow!("No suitable tokens found for rotation"));
        }
        
        info!("Found {} tokens for rotation", filtered_tokens.len());
        
        // Log the tokens we'll be using
        ...
        let mut token_list = self.token_list.lock().unwrap();
    ...
    pub fn initialize_thread_tokens(&self, thread_count: usize) -> Result<()> {
        ...
        // Clear any existing indices and history
        thread_indices.clear();
        thread_history.clear();
        
        // Initialize token history for each thread
        for _ in 0..thread_count {
            thread_history.push(Vec::new());
        }
        
        // Assign initial indices to threads, distribute them across the token list
        for thread_id in 0..thread_count {
            // Calculate a starting index for each thread that ensures they start with different tokens
            let index = (thread_id % token_list.len());
            thread_indices.push(index);
            
            // Add this index to the thread's history
            thread_history[thread_id].push(index);
            
            info!("Thread {} assigned initial token: {} ({})", 
                thread_id, 
                token_list[index].symbol, 
                token_list[index].address);
        }
    ...
    pub fn get_token_for_thread(&self, thread_id: usize) -> Result<Token> {
        let token_list = self.token_list.lock().unwrap();
        let thread_indices = self.thread_token_indices.lock().unwrap();
        ...
        let token_index = thread_indices[thread_id];
        let token = token_list[token_index].clone();
